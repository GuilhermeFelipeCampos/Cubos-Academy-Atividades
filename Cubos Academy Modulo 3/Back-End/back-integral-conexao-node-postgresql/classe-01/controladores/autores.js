...
const excluirAutor = async (req, res) => {
    ...
    try {
        ...
        const livroAutor = await conexao.query('select * from livros where autor_id = $1', [id]);
        console.log(livroAutor.rowCount)
        if(livroAutor.rowCount !== 0 ){
            return res.status(400).json('Não foi possível excluir o autor, existem livros atrelados!');
        }
