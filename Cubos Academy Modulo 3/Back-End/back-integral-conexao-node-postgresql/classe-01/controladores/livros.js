...
const excluirLivro = async (req, res) => {
    ...
    try {
        ...
        const livroEmprestimo = await conexao.query('select * from emprestimos where livro_id = $1', [id]);
        console.log(livroEmprestimo.rowCount)
        if(livroEmprestimo.rowCount !== 0 ){
            return res.status(400).json('Não foi possível excluir o livro, existem empréstimos atrelados!');
        }
